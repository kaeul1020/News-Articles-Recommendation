...
for user in range(1, 5):
    text = []
    data = pd.read_excel(f'News-Articles-Recommendation/ko_model/news_user{user}.xlsx')
...
model = tp.LDAModel(k=2, alpha=0.1, eta=0.01, min_cf=4)
# 전체 말뭉치에 4회 미만 등장한 단어들은 제거할 겁니다.
 
for line in list(df['ptexts']):
...
recommend.to_csv('News-Articles-Recommendation/ko_model/recommend.csv',index=None)
